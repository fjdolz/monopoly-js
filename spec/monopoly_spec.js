...
describe("Pasar por la casilla de Salida", function(){
	...
	it("Cobra 200 pelotis al pasar por la casilla de salida",function(){
		...
		usr.fichas[0].posicion=38
		
		var saldoinicial = usr.fichas[0].saldo;
		var num= 3; //das una vuelta entera
		if ((usr.fichas[0].getPosicion() + num)>39) {
			var nuevaPosicion = (usr.fichas[0].getPosicion()  + num)-40;
			usr.fichas[0].pasarsalida();
			
		}
		
		partida.moverFicha(usr.fichas[0], 3);
...
describe("Fases", function(){
	...
	it("En fase de Inicio solo se pueden agregar usuarios", function(){
		...
		expect(usr.unirseAPartida(partida)).toEqual(true)
		expect(partida.lanzarDados()).toEqual(-1) //error cuando no se encuentra en su fase de ejecucion
	});
	...
	it("En la fase de Juego solo se deja lanzar dados y transaciones", function(){
		...
		expect(partida.lanzarDados()).not.toEqual(-1) //error que lanza cuando no esta en su fase
		expect(partida.estado).toEqual("jugar")
		expect(usr.unirseAPartida(partida)).toEqual(-1)
	...
	it("Al lanzar el juego,se encuentra en la Fase inicial", function(){
		...
		expect(partida.estado).toEqual("inicio")
